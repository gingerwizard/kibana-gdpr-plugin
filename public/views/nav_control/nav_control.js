...
chromeNavControlsRegistry.register(constant({
  name: 'kibana-gdpr-plugin',
  order: 1000,
  template
}));

const module = uiModules.get('kibana-gdpr-plugin', ['kibana']);
  ...
  if (document.cookie.split(';').filter((item) => {
      ...
      let req = $.ajax({ dataType: "json", url: 'https://api.ipify.org?format=jsonp&callback=?',
          success: function( data ) {
            ...
            $http.get(basePath+'/api/kibana-gdpr-plugin/ip_info?ip='+data.ip).then(function mySuccess(data) {
